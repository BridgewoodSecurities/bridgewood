...
class Trade(Base):
    ...
    client_order_id: Mapped[str] = mapped_column(String(255), unique=True, nullable=False)
...
class Position(Base):
    ...
    __table_args__ = (UniqueConstraint("agent_id", "symbol", name="uq_positions_agent_symbol"),)
...
class ActivityLog(Base):
    ...
    id: Mapped[int] = mapped_column(primary_key=True, autoincrement=True)
    agent_id: Mapped[str] = mapped_column(String(36), ForeignKey("agents.id"), nullable=False, index=True)
    event_type: Mapped[ActivityEventType] = mapped_column(SqlEnum(ActivityEventType), nullable=False)
