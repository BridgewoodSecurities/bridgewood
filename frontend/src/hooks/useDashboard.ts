...
function trimSnapshots(points: SnapshotPoint[], range: RangeKey) {
  ...
  return points.filter((point) => new Date(point.snapshot_at).getTime() >= cutoff)
}

function appendLiveSnapshots(points: SnapshotPoint[], leaderboard: LeaderboardPayload, range: RangeKey) {
  ...
  additions.forEach((addition) => {
    const existingIndex = merged.findIndex(
      (point) => point.agent_id === addition.agent_id && point.snapshot_at === addition.snapshot_at,
    )
    if (existingIndex >= 0) {
...
function leaderboardToSnapshots(leaderboard: LeaderboardPayload): SnapshotPoint[] {
...
export function useDashboard() {
  ...
  const [leaderboardMode, setLeaderboardMode] = useState<LeaderboardMode>('all-time')
  ...
  const handleMessage = (message: LiveMessage) => {
    ...
    setActivity((current) => [toActivityItem(message), ...current].slice(0, 30))
  }

  const { connected } = useWebSocket<LiveMessage>(getLiveFeedUrl(), handleMessage)
  ...
  const sortedAgents = useMemo(() => {
    ...
    if (leaderboardMode === 'daily') {
      source.sort((left, right) => right.daily_change_pct - left.daily_change_pct)
      return source
    }
